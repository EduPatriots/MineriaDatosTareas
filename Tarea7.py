...
df_by_sal = disparidad_anual
df_by_sal.reset_index(inplace=True)
a = linear_regression(df_by_sal, "schedule_season", "spread_favorite")
plt_lr(df=df_by_sal, x="schedule_season", y="spread_favorite", colors=('red', 'orange'), **a)

plt.xticks(rotation=90)
plt.savefig('lr_año_favorito_m.png')
plt.close()